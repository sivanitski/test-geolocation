class GeolocationsController < ApplicationController
    ...
    if !@geolocation.present?
      begin
        ...
        if geolocation_data
          ...
          if @geolocation.save
            render json: serialized('geolocation', @geolocation, { include: [:location] }), status: 201
          else
      ...
      render json: serialized('geolocation', @geolocation, { include: [:location] }), status: 302
    end
  end
  ...
  def show
    ...
    if @geolocation.present?
      ...
      render json: { error: 'Could not find geolocation with the IP' }, status: 404
    end
  end
  ...
  def destroy
    ...
    if @geolocation&.destroy
      render json: { message: 'Geolocation destroyed successfully' }, status: 200
    else
